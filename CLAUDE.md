...
- `state.json` (committed to the repo) tracks, per CIK, which accession numbers have already produced a notification, so re-runs don't re-notify. Filings that don't match a tracked title or have no qualifying transaction are *not* recorded, so they get re-fetched and re-checked on every subsequent run. It also holds a top-level `_last_heartbeat` ISO timestamp (see below), separate from the per-CIK accession lists.
- `send_email` / `send_heartbeat_email` both go through `send_resend_email`, which posts to the Resend API. Recipient and sender are hardcoded constants (`NOTIFY_EMAIL`, and Resend's sandbox address `onboarding@resend.dev`) — the sender does not need to be the recipient's own address. Requires `RESEND_API_KEY` in the environment to actually send; without it (e.g. local `--dry-run` or an unset key), notifications are computed and printed but the send is skipped.
- If a run finds no qualifying transactions and it's been 7+ days (`HEARTBEAT_INTERVAL`) since `_last_heartbeat`, it sends a "still watching, no new filings" heartbeat email instead of staying silent — this exists so that weeks without a real alert don't get mistaken for the bot being broken. `_last_heartbeat` is only updated when the heartbeat actually sends (i.e. not on `--dry-run`, and not if `RESEND_API_KEY` is missing), so a misconfigured key can't silently "use up" a week's heartbeat.
